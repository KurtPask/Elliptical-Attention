...
parser.add_argument('--attenuation', type = float, default = 1e-3, help = 'attenuation factor in W_over_layers computation')
...
if args.restart:
    ...
    model = MemTransformerLM(
        ...
        compare_downsample_grads = args.compare_downsample_grads,
        smoothing_correction = args.smoothing_correction,
...
def train():
    ...
    for batch, (data, target, seq_len) in enumerate(train_iter):
        ...
        if args.batch_chunk > 1:
            ...
            #         data, target, *mems,
            #         carry_over_fast_weight=args.carry_over_fast_weight)
            #     if (batch == (len(train_iter) - 1)) & (epoch % 5 == 0):
            #         logging(f"First sequence in batch, first head, function gradient estimates {torch.diag(W[0][0])}")
            ...
            if args.show_M:
                for idx, layer in enumerate(para_model.module.layers):
                    if idx in args.M_positions:
                        ...
                        if hasattr(layer.dec_attn, 'diff_quotients'):
                            diff_quotients = layer.dec_attn.diff_quotients
                        ...
                        # if use_wandb:
                        #     for i, value in enumerate(layer.dec_attn.W):
                        #         wandb.log({f'Avg_Grad_{i+1}': value})
                        #         #print(f'Avg_Grad_{i+1}: {value:.5f}')
