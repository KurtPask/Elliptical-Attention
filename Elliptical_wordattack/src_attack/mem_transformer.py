...
class MultiHeadAttn(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, dropatt=0, 
                 ...
                 over_layers = False, attenuation = 1e-3, median = False, ablation = False):
    ...
    def W_over_layers(self, head_q, head_q_last, head_v, head_v_last, delta = None, attenuation = 1e-3, median = False):
        with torch.no_grad():
            ...
            if median:
                W = torch.median(torch.abs(difference_quotients), dim = 0)[0]
            else:
    ...
    def forward(self, h, attn_mask=None, mems=None,
        ...
        if self.M: # attention with gradient weights
            if self.ablation:
                ...
                #breakpoint()
                W = self.W_over_layers(head_q, head_q_last, head_v, head_v_last, delta = 5, attenuation = self.attenuation)
            else:
                ...
                if i in M_positions: # append an M-decoder layer 
                    self.layers.append(
                        DecoderLayer(
                            ...
                            attenuation = attenuation, median = median, ablation = ablation)
                    )
