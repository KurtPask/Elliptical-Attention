...
default_cfgs = {
    # Patch size 16
    'elliptical_fan_tiny_12_p16_224': _cfg(),
    'elliptical_fan_small_12_p16_224': _cfg(),
...
class EllipticalTokenMixing(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., 
                    sr_ratio=1, linear=False, share_atten=False, drop_path=0., emlp=False, sharpen_attn=False,
                    mlp_hidden_dim=None, act_layer=nn.GELU, drop=None, norm_layer=nn.LayerNorm, M = False, over_layers = False, attenuation = 5e-1,
                    median = False, show_M = False):
        super().__init__()
        assert dim % num_heads == 0, f"dim {dim} should be divided by num_heads {num_heads}."
    ...
    def W_over_layers(self, head_q, head_q_last, head_v, head_v_last, delta = None, attenuation = 1e-3, median = False):
        with torch.no_grad():
            ...
            if delta is None:
                deltas = torch.abs(head_q - head_q_last) + attenuation #include term for stability and gradient attenuation
            ...
            if median: # robustify the average gradient computation to extreme outliers by taking a median
                W = torch.median(torch.abs(difference_quotients), dim = -2)[0]
            else:
    ...
    def forward(self, x, H, W, q_last = None, v_last = None, atten=None, return_attention=False):
        B, N, C = x.shape
        q = self.q(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        ...
        if self.M:
            if self.over_layers:
                Wt = self.W_over_layers(q, q_last, v, v_last, delta = 5, attenuation = self.attenuation, median = self.median)
                #Wt = self.W_over_layers(k, q_last, v, v_last, attenuation = self.attenuation, median = self.median) # kv
            else:
                Wt = self.compute_W(q,k,v, t = self.t)
            QW = q @ Wt #[bsize x nhead x qlen x dhead]
            attn = QW @ k.transpose(-2,-1)*self.scale
...
class EllipticalFANBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0., sharpen_attn=False,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, eta=1., sr_ratio=1., downsample=None, c_head_num=None, M = False,
                 over_layers = False, median = False, attenuation = 5e-1, show_M = False):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = EllipticalTokenMixing(dim, num_heads=num_heads, qkv_bias=qkv_bias, mlp_hidden_dim=int(dim * mlp_ratio), sharpen_attn=sharpen_attn,
                                    attn_drop=attn_drop, proj_drop=drop, drop=drop, drop_path=drop_path, sr_ratio=sr_ratio, M = M, over_layers = over_layers,
                                    median = median, attenuation = attenuation, show_M = show_M)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
    ...
    def forward(self, x, q_last = None, v_last = None, attn=None, return_attention=False):
        H, W = self.H, self.W

        if self.over_layers:
            x_new, q_last, v_last, attn_s = self.attn(self.norm1(x), H , W, q_last, v_last)
        else: # normal pass forward
            x_new, attn_s = self.attn(self.norm1(x), H, W)
        x = x + self.drop_path(self.gamma1 * x_new)
...
class EllipiticalFAN(nn.Module):
    """
    Based on timm code bases
    https://github.com/rwightman/pytorch-image-models/tree/master/timm
    """
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12, sharpen_attn=False, channel_dims=None,
                 num_heads=12, mlp_ratio=4., qkv_bias=True, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.,sr_ratio=None, backbone=None, use_checkpoint=False,
                 act_layer=None, norm_layer=None,se_mlp=False, cls_attn_layers=2, use_pos_embed=True, eta=1., tokens_norm=False, c_head_num=None, hybrid_patch_size=2, head_init_scale=1.0,
                 M = False, over_layers = False, median = False, attenuation = 5e-1, M_positions = [], show_M = False):
        ...
        if se_mlp:
            ...
            build_block = EllipticalFANBlock
        self.blocks = nn.ModuleList([])
        for i in range(depth):
            if i < depth - 1 and channel_dims[i] != channel_dims[i+1]:
                downsample = OverlapPatchEmbed(img_size=img_size,
            ...
            if i in M_positions:
                self.blocks.append( 
                    build_block( # append elliptical block - M on
                        dim=channel_dims[i], num_heads=num_heads[i], mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate, sr_ratio=sr_ratio[i], 
                        attn_drop=attn_drop_rate, drop_path=drop_path_rate, act_layer=act_layer, norm_layer=norm_layer, eta=eta, 
                        downsample=downsample, c_head_num=c_head_num[i] if c_head_num is not None else None, M = True, over_layers = over_layers,
                        attenuation = attenuation, median = median, show_M = show_M))
            else:
                self.blocks.append( 
                    build_block( # append elliptical block with M off - i.e normal attention block
                        dim=channel_dims[i], num_heads=num_heads[i], mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate, sr_ratio=sr_ratio[i], 
                        attn_drop=attn_drop_rate, drop_path=drop_path_rate, act_layer=act_layer, norm_layer=norm_layer, eta=eta, 
                        downsample=downsample, c_head_num=c_head_num[i] if c_head_num is not None else None, M = False, over_layers = over_layers))
    ...
    def forward_features(self, x, q_last = None, v_last = None):
        B = x.shape[0]
        x, (Hp, Wp) = self.patch_embed(x)
        ...
        for blk in self.blocks:
            blk.H, blk.W = H, W
            if not self.over_layers: # normal pass through
                if self.use_checkpoint:
                    x = checkpoint.checkpoint(blk, x)
                else:
                    x = blk(x)
            else: # overlayers pass through
                if self.use_checkpoint:
                    x, q_last, v_last = checkpoint.checkpoint(blk, x, q_last, v_last)
                else:
                    x, q_last, v_last = blk(x, q_last, v_last)
...
def _create_elliptical_fan(variant, pretrained=False, default_cfg=None, **kwargs):
    default_cfg = default_cfg or default_cfgs[variant]
    model = build_model_with_cfg(
        EllipiticalFAN, variant, pretrained, default_cfg=default_cfg, pretrained_filter_fn=checkpoint_filter_fn, **kwargs)
    return model


@register_model
def elliptical_fan_tiny_12_p16_224(pretrained=False, bn_tf=False, **kwargs):
    depth = 12
    ...
    model = _create_elliptical_fan('elliptical_fan_tiny_12_p16_224', pretrained=pretrained, over_layers = True, sr_ratio=sr_ratio, **model_kwargs)
    return model

@register_model
def elliptical_fan_small_12_p16_224(pretrained=False, bn_tf=False, **kwargs):
    depth = 12
    sr_ratio = [1] * depth
    model_kwargs = dict(
        patch_size=16, embed_dim=384, depth=depth, num_heads=8, eta=1.0, tokens_norm=True, **kwargs)
    model = _create_elliptical_fan('elliptical_fan_small_12_p16_224', pretrained=pretrained, over_layers = True, sr_ratio = sr_ratio, **model_kwargs)
    return model

@register_model
def elliptical_method1_fan_tiny_12_p16_224(pretrained=False, bn_tf=False, **kwargs):
    depth = 12
    sr_ratio = [1] * (depth//2) + [1] * (depth//2)
    model_kwargs = dict(
        patch_size=16, embed_dim=192, depth=depth, num_heads=4, eta=1.0, tokens_norm=True, sharpen_attn=False, **kwargs)
    model = _create_elliptical_fan('elliptical_method1_fan_tiny_12_p16_224', pretrained=pretrained, sr_ratio=sr_ratio, over_layers = False, **model_kwargs)
    return model
