    ...
    def forward(self, x, head_q_last = None, head_v_last = None):
        ...
        if self.M: # mahlattention computation
            if self.ablation:
                ...
                W = self.W_over_layers(head_q, head_q_last, head_v, head_v_last, attenuation = self.attenuation, median = self.median)
            else:
...
@register_model
def mahala_deit_tiny_patch16_224(pretrained=False, **kwargs):
...
@register_model
def mahala_deit_tiny_patch16_224_wrapper(pretrained=False, pretrained_cfg=None, **kwargs):
    del kwargs['robust']
    # del kwargs['no_embed_class']
    return mahala_deit_tiny_patch16_224(pretrained=pretrained, **kwargs)

@register_model
def mahala_deit_overlayers_tiny_patch16_224(pretrained=False, **kwargs):
    model = MahalaVisionTransformer(
...
@register_model
def mahala_deit_overlayers_tiny_patch16_224_wrapper(pretrained=False, pretrained_cfg=None, **kwargs):
    del kwargs['robust']
    # del kwargs['no_embed_class']
    return mahala_deit_overlayers_tiny_patch16_224(pretrained=pretrained, **kwargs)